...
Repo.prototype.uploadPack = function (readRefUpdates, readPack, readIdx, cb) {
  if (this.sbot.id && this.sbot.id != this.feed)
    return cb(new Error('You do not have permission to push to this repo'))

  var done = multicb({ pluck: 1, spread: true })
  pull(readPack, addSSBBlob(this.sbot, done()))
  pull(readIdx, addSSBBlob(this.sbot, done()))

  if (readRefUpdates)
    Repo_readRefUpdates.call(this, readRefUpdates, done())

  var self = this
  done(function (err, packId, idxId, refUpdatesObj) {
    if (err) return cb(err)
    self.sbot.publish({
      type: 'git-update',
      repo: this.id,
